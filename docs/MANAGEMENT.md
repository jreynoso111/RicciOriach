...
Crear el evento con fecha, ciudad y lugar. Para publicar boletas disponibles o agotadas, añadir la plataforma externa (por ejemplo, Ticketmaster o Tix) y su URL HTTPS directa. Indicar el estado y, opcionalmente, el stock o las etapas visibles (por ejemplo, «Quedan 12 · VIP agotado»). Ese dato se actualiza manualmente según lo que informe la boletera; no existe sincronización de inventario en tiempo real. El botón público lleva directamente a la plataforma externa. El gestor no crea ni vende entradas.
...
Los visitantes seleccionan productos, completan contacto y aceptan el uso de esos datos para gestionar la solicitud. Reciben una referencia. No se envía correo automático ni se cobra al enviar el formulario. La solicitud no bloquea unidades hasta que un administrador la confirme. La compra de taquillas ocurre en la plataforma vinculada a cada evento.

En Pedidos de tienda: Pendiente → Confirmado → Entregado, o Pendiente/Confirmado → Cancelado. Confirmar asigna inventario de forma transaccional. Cancelar libera unidades. Los pagos manuales se registran separadamente y solo deben marcarse recibidos cuando el cobro esté comprobado. Se conservan las referencias, datos del comprador, precios de la compra y notas internas.
...
El webhook es `/api/paypal-webhook`. Suscribir `CHECKOUT.ORDER.APPROVED`, `PAYMENT.CAPTURE.COMPLETED`, `PAYMENT.CAPTURE.DENIED` y `PAYMENT.CAPTURE.REFUNDED`. Verificar una compra sandbox de tienda, cancelación, firma inválida, notificación repetida y reembolso antes de activar producción.
...
El servidor calcula el precio, reserva inventario de productos, crea el pedido PayPal y entrega el enlace seguro. La página de retorno y los webhooks consultan PayPal para verificar referencia, importe, moneda y captura completada. Las respuestas duplicadas no descuentan más unidades. Los pagos en proceso no liberan inventario por expiración. Las reservas de pago no iniciadas vencen después de tres horas; la limpieza se ejecuta al consultar la tienda o comenzar otro checkout. Los procesos ambiguos permanecen para revisión y reintento, sin asumir que fallaron.

Reembolsos en el portal PayPal; el webhook actualiza el estado. Un reembolso no repone automáticamente productos ya enviados; el equipo revisa la devolución e inventario. No se almacenan números de tarjeta.
...
`npm test` ejecuta validaciones de enlaces, comportamiento, el gestor local, 38 comprobaciones de comercio y 16 de avisos mediante PostgreSQL real con PGlite, además de las comprobaciones de PayPal desactivado y validación de importes. `node scripts/preview_manager.mjs` inicia una prueba aislada en 4191, con datos ficticios y sin acceso a Auth, Storage o PayPal reales. No debe publicarse ni confundirse con el gestor de producción. La agenda se revisó visualmente con enlaces externos, estado y stock de muestra; sus enlaces no llevan a una boletera real.

Las migraciones de gestión y PayPal fueron aplicadas al proyecto remoto mediante su SQL Editor. Conservar estos archivos como registro de esquema; no reaplicarlos sobre tablas existentes sin revisar el historial. La migración `20260913000629_event_external_ticketing.sql` añade los datos de boletera/stock y bloquea nuevas ventas internas; debe aplicarse al proyecto antes de editar esos campos desde el panel.
